...
sourceId: "source:ck3-paradoxwikis-com:traits"
...
- Source: `source:ck3-paradoxwikis-com:traits`
- Revision: <https://ck3.paradoxwikis.com/Traits?oldid=34781>
...
Use this page when a question needs CK3 mechanics context for Traits. Combine these mechanics notes with current-save evidence paths and advisor models before giving player advice.
...
- Congenital traits have a very small chance to appear upon birth and can be inherited by a character's descendants, especially if both parents have the same trait or are related. Congenital traits are marked with a white aura.

### Leveled congenital traits

- The following Congenital traits come in multiple levels. A leveled Congenital trait has a 50% chance to increase in level if both parents have the same trait, both positively and negatively. The game incorrectly states that if both parents have the same trait, it will get inherited (it will, just not as an active trait), the actual factors for genetic inheritance are complicated, see Family (relation).
...
- A Health trait is a negative trait that lowers a character's Health and is visible on the character. They last a varying amount of time and differ greatly in lethality. The visual effect of certain health traits can be replaced with one from a lesser trait by disabling Show graphic diseases and injuries option in the settings menu.
...
- Diseases are Health traits that can be spread from one character to another. If a character recovers from a disease they become immune to it. Immunity is permanent for the Bubonic Plague, Smallpox and Measles and lasts 20 years for other traits. There is no immunity against the Ill trait. STD diseases are congenital and can also be spread to other characters if sleeping with them.

### Old age traits

- Old age traits are added to characters who are elderly enough, with the chance increasing the older the character gets. These traits progress through 4 levels but progression can be slowed down. Each trait level adds to the previous ones. Which traits are available depends on the Elderly Health game rule but can be added to custom characters regardless, where they cost -20 customization points.
...
- Mechanics content slug: `traits`
- Source title: `Traits`
- Source page length: `159170`
